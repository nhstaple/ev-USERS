...
let data: IVocab = VocabExample;
const LOG = false;
if(LOG) {
    console.log(`example data: `, data);
}
const target_query_value: string = 'la cuchara';
const target_update_attribute = { value: 'the spanish inquisition' };

const CLEAN_UP = false;

describe('db.api update', () => {
    ...
    it('updating to a table', async() => {
        expect.assertions(1);
        // console.log('updating rows in a table');
        expect(await client.update(DB_NAME, TABLE_NAME, data, target_update_attribute)).toBeTruthy();
    });

    it('verifying the table updated (re-running query)', async() => {
        expect.assertions(1);
        // console.log('querying from a table');
        const items = await client.query(DB_NAME, TABLE_NAME, {});
        // console.log(items);
        expect(items[0]['value']).toBe(target_update_attribute['value']);
    });
